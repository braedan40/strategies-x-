...
local MainLink = LoadLocal and "" or "https://raw.githubusercontent.com/braedan40/strategies-x-/refs/heads/main/TDS/"
	...
	task.spawn(function()
		...
		StratXLibrary.SignalMatchEnd = MatchGui:GetPropertyChangedSignal("Visible"):Connect(function()
			...
			if UtilitiesConfig.RestartMatch and RSHealthCurrent.Value == 0 then --StratXLibrary.RestartCount <= UtilitiesConfig.RestartTimes
				...
				task.wait(5)
				getgenv().OldPickups = nil -- removed the operation because it's gone--LocalPlayer.PlayerGui:WaitForChild("ReactOverridesTopBar"):WaitForChild("Frame"):WaitForChild("items"):WaitForChild("Operation I.C.E"):WaitForChild("text").Text
				StratXLibrary.ReadyState = false
